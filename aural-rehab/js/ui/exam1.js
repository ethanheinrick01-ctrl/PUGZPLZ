...
(function(root){
  ...
  function overview(){
    ...
    h+='<div class="row">'+link('mocks','Start a full learning mock','pri')+link('review','Review '+miss.length+' open misses')+'</div><h2>Work through the focused chapters</h2><p class="small muted">Newly checked graded answers, including full mocks, feed concept progress. Two correct, unhinted answers on different questions, with medium/high confidence on the second, earn lasting mastery. Misses still go to review. Existing saved history is preserved.</p><div class="grid g3">'+D.chapters.map(chapterCard).join('')+'</div>';
    html(h);
  }
  function chapters(id){
    ...
    h+='<details class="card"><summary>Concept progress: '+st.mastered+'/'+st.total+'</summary><p class="small muted">These are shared concepts from your existing progress record. Earned mastery stays; a later miss is still kept in your review queue.</p><ul>'+st.concepts.map(function(k){var s=stats[k];return '<li>'+esc(L.CONCEPTS[k].name)+' — '+esc(s.status)+(s.masterySource==='self'?' (self-marked)':'')+((s.rawStatus==='shaky'||s.rawStatus==='misconception')&&s.status==='mastered'?' · recent miss for review':'')+' ('+s.att+' graded attempts)</li>';}).join('')+'</ul></details>';
