...
(function(root){
  ...
  function overview(){
    ...
    if(active) {var sc=X.score(active);h+='<div class="card"><b>'+esc(active.title)+'</b><p>'+sc.answered+'/'+sc.count+' questions checked · last saved '+esc(date(active.updated))+'</p>'+link('run/'+active.id,'Resume exactly where I left off','good')+'</div>';}
    h+='<div class="row">'+link('mocks','Start a full learning mock','pri')+link('review','Review '+miss.length+' open misses')+'</div><h2>Work through the focused chapters</h2><p class="small muted">Mastery still requires two correct, unhinted answers on different questions, with medium/high confidence on the latest. Reading a chapter or completing a mock does not grant mastery.</p><div class="grid g3">'+D.chapters.map(chapterCard).join('')+'</div>';
    html(h);
  }
  function chapters(id){
    ...
    h+='<details class="card"><summary>Concept progress: '+st.mastered+'/'+st.total+'</summary><p class="small muted">These are shared concepts from your existing progress record. An open mock miss takes the concept out of this chapter’s ready count until corrected in practice.</p><ul>'+st.concepts.map(function(k){var s=stats[k];return '<li>'+esc(L.CONCEPTS[k].name)+' — '+esc(s.status)+' ('+s.att+' practice attempts)</li>';}).join('')+'</ul></details>';
  ...
  function runList(runs){if(!runs.length)return '<p class="muted">No runs yet.</p>';return '<div class="e1runs">'+runs.slice().reverse().map(function(r){var sc=X.score(r);return '<div class="card spread"><div><b>'+esc(r.title)+'</b><p class="small">'+esc(date(r.started))+' · '+sc.answered+'/'+sc.count+' checked · '+sc.total+'/'+sc.possible+' practice points'+(sc.pendingWriting?' · writing still pending':'')+'</p></div><div class="row">'+link('run/'+r.id,'Resume / inspect')+link('result/'+r.id,'Progress summary')+'</div></div>';}).join('')+'</div>';}
  ...
  function review(){var misses=X.misses();html(shell('Review misses','A wrong answer joins this queue immediately. Relearn it here, then answer distinct practice questions correctly to rebuild mastery.')+(misses.length?'<div class="grid g3">'+misses.map(function(m,i){return '<article class="card"><span class="badge">'+esc(X.chapter(m.chapter).title)+'</span><h3>'+esc(L.CONCEPTS[m.c]?L.CONCEPTS[m.c].name:'Written response')+'</h3><p class="small">'+esc(date(m.time))+'</p><div class="row"><button class="btn pri" data-retry="'+i+'">Practice this now</button>'+link('run/'+m.run+'/'+m.index,'Revisit feedback')+link('chapters/'+m.chapter,'Relearn')+'</div></article>';}).join('')+'</div>':'<div class="card">No open Exam 1 misses yet. Misses will appear here as you answer; an empty queue is not evidence of mastery before you have practiced.</div>'));main.querySelectorAll('[data-retry]').forEach(function(b){b.onclick=function(){go('run/'+X.retry(misses[+b.dataset.retry]).id);};});}
  function run(id,at){
